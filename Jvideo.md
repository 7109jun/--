...
여기에 .jta 텍스트 전체를 붙여넣으세요"></textarea>
      ...
      <div class="row">
        <button id="dlWavBtn" class="ghost" disabled>⬇ WAV로 다운로드</button>
        <button id="dlMp3Btn" class="ghost" disabled>⬇ MP3로 다운로드</button>
      </div>
      <div class="muted" id="mp3StatusNote" style="margin-top:4px;"></div>
...
let lastDecodedBuffer = null, lastDecodedTitle = null;
...
function renderDecoded(decoded){
  ...
  // NEW: keep the reconstructed audio around so it can be exported as a
  // normal WAV or MP3 file once playback has been confirmed to work.
  lastDecodedBuffer = buf;
  lastDecodedTitle = (decoded.meta.title || "jtaudio_output").replace(/[^\w\-]+/g, "_");
  document.getElementById("dlWavBtn").disabled = false;
  document.getElementById("dlMp3Btn").disabled = (typeof lamejs === "undefined");
  document.getElementById("mp3StatusNote").textContent = (typeof lamejs === "undefined")
    ? "⚠ MP3 인코더를 불러오지 못했습니다(인터넷 연결 필요) — WAV 다운로드는 정상 동작합니다."
    : "";
...
function bufferToMp3Blob(buf){
  if (typeof lamejs === "undefined") throw new Error("MP3 인코더(lamejs)를 불러오지 못했습니다. 인터넷 연결을 확인해주세요.");
  const nCh = buf.numberOfChannels, sr = buf.sampleRate, len = buf.length;
  const mp3encoder = new lamejs.Mp3Encoder(nCh, sr, 128); // 128kbps, standard default
  const sampleBlockSize = 1152; // required block size for lamejs
  const left = floatTo16BitPCM(buf.getChannelData(0));
  const right = nCh > 1 ? floatTo16BitPCM(buf.getChannelData(1)) : null;
  const mp3Data = [];
...
document.getElementById("dlWavBtn").addEventListener("click", () => {
  if (!lastDecodedBuffer) return;
  downloadBlob(bufferToWavBlob(lastDecodedBuffer), (lastDecodedTitle || "output") + ".wav");
});
document.getElementById("dlMp3Btn").addEventListener("click", () => {
  if (!lastDecodedBuffer) return;
  try{
    downloadBlob(bufferToMp3Blob(lastDecodedBuffer), (lastDecodedTitle || "output") + ".mp3");
  }catch(err){
    alert("MP3 변환 실패: " + err.message);
  }
});
